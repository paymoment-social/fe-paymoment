...
export function PostDetailView({ postId }: { postId: string }) {
  const feed = useFeed();
  if (feed.isLoading) return <div className="space-y-4 py-6"><Skeleton className="h-14 w-full" /><Skeleton className="h-[30rem] rounded-xl" /><Skeleton className="h-24 rounded-xl" /></div>;
  if (feed.isError) return <main className="grid min-h-[60vh] place-items-center px-4"><section className="max-w-md rounded-xl border border-destructive/30 bg-destructive/5 p-8 text-center"><Icon icon="solar:danger-triangle-linear" className="mx-auto size-10 text-destructive" aria-hidden="true" /><h1 className="mt-4 text-xl font-semibold">Couldn’t load this moment</h1><p className="mt-2 text-sm text-muted-foreground">Check your connection and try again.</p><Button variant="outline" className="mt-5 h-10" onClick={() => void feed.refetch()}>Try again</Button></section></main>;
