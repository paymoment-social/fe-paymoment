...
export function DiscoverView() {
  ...
  return <div className="space-y-4">
    ...
    {data && <>
      {(filter === "all" || filter === "people") && data.people.length > 0 && <section className="rounded-xl border bg-card/55 p-5"><h2 className="font-semibold">{query.trim() ? "People" : "People to follow"}</h2><div className="mt-4 grid gap-3 sm:grid-cols-2">{data.people.slice(0, filter === "all" && !query.trim() ? 4 : undefined).map((person) => { const following = person.relationship === "following"; return <div key={person.id} className="flex items-center gap-3 rounded-xl bg-secondary/60 p-3"><AuthorAvatar author={person} /><div className="min-w-0 flex-1"><div className="flex items-center gap-1"><p className="truncate text-sm font-medium">{person.name}</p>{person.verified && <VerifiedMark />}</div><p className="truncate text-xs text-muted-foreground">@{person.handle}</p></div><Button variant={following ? "outline" : "default"} size="sm" className="h-10 px-3" disabled={follow.isPending} onClick={() => follow.mutate({ userId: person.id, enabled: !following })}>{following ? "Following" : "Follow"}</Button></div>; })}</div></section>}
