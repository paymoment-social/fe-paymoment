...
type ApiCatalog = { id: string; slug: string; title: string; description: string; costPoints: number; inventory: number | null; active: boolean };
const iconBySlug: Record<string, string> = { "verified-badge": "solar:verified-check-bold", "profile-highlight": "solar:stars-bold" };
function mapCatalog(item: ApiCatalog): RewardItem { return { id: item.id, slug: item.slug, title: item.title, description: item.description, cost: item.costPoints, icon: iconBySlug[item.slug] ?? "solar:gift-bold", available: item.active && (item.inventory === null || item.inventory > 0) }; }
