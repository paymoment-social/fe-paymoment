...
export function MessagesView() {
  ...
  return <div className="flex h-full min-h-0 flex-col gap-3">
    ...
    {active && (activeTyping.length > 0 || activePresence) && <p role="status" className="flex items-center justify-end gap-1.5 text-right text-xs text-muted-foreground">{activeTyping.length > 0 ? <>{active.user.name} is typing...</> : <><span className={`size-2 rounded-full ${activePresence === "available" ? "bg-emerald-400 shadow-[0_0_0_3px_rgba(52,211,153,0.12)]" : "bg-amber-400"}`} aria-hidden="true" />{activePresence === "available" ? `${active.user.name} is available` : `${active.user.name} is away`}</>}</p>}
